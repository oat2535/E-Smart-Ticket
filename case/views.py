...
def case(request):
    ...
    if request.user.username == "admin" :  # ถ้าเป็นแอดมิน ให้ดูทั้งหมด
        ...
        caseCountDone = base_qs.filter(status_id=5).count()
        caseCountSatisfied = base_qs.filter(status_id=4).count()
        ...
        caseCountDone = base_qs.filter(status_id=5).count()
        caseCountSatisfied = base_qs.filter(status_id=4).count()
        caseCountReceive = base_qs.filter(status_id=7, department_id="PUR").count()
        caseCountIT = base_qs.filter(department_id="IT").count()
        caseCountPUR = base_qs.filter(department_id="PUR").count()
        caseCountFIN = base_qs.filter(department_id="FIN").count()
